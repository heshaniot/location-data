...
public class LocationDataProducer {
    ...
    public Logger log = Logger.getLogger(LocationDataProducer.class);
    ...
    public static void main(String[] args) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				double lattitude = Double.parseDouble(location[3]);

				GeoJsonPointFeature geoJsonPointFeature =
				                                          new GeoJsonPointFeature(deviceId,
				                                                                  timeStamp);
				GeoJsonPoint geoJsonPoint = new GeoJsonPoint(longitude, lattitude);
